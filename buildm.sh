...
if [ -d "anykernel/.git" ]; then
    ...
    if [ -f "out/arch/arm64/boot/Image.gz" ]; then
        ...
        echo "The file [out/arch/arm64/boot/Image.gz] does not exist. Seems like build failed."
        exit 1
    fi
    ...
    cd anykernel
    ZIP_FILENAME=Kernel_BloodReaper_pipa_MIUI_anykernel3_${GIT_COMMIT_ID}.zip
